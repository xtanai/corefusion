...
* **Adaptive bandwidth balancing:** Optimized for **1 GbE** streaming (typical setups use **10–20 GbE**, NIC-dependent). Each stereo rig adapts **ROI coverage and update rate** to link budget and visibility. If one rig degrades (occlusion/blur), a neighboring rig briefly **expands coverage** to compensate, then hands back once quality recovers—preserving stable, accurate 3D pose under bandwidth constraints.
